...
def main():
    ...
    ap.add_argument("--out", default="baseline-out/recursive.out")
    args = ap.parse_args()
    ...
    for index, (name, exp) in enumerate(expected):
        ...
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'} {name} [{result.elapsed_ms} ms] {result.outcome} {result.first[:110]}")
