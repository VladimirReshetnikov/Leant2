# Closed-query library API and portable refutation checkpoint

Both complete runs passed **832/832 required checks across 13 families** at clean source revision `9ec21c81471a10dd72e56a1f330d502cfb19465d`, using `leanprover/lean4:v4.34.0`. The configured denominator and observed complete scores agree. Each aggregate run records a successful full build; the frontend runner itself does not build. Query budgets are cooperative and do not impose a strict whole-process time limit.

The two retained aggregate build logs each record **68 successful build jobs**
for the library, native tests, and executable. The API and refutation tests are
part of that build gate; they add no external harness family or acceptance case.
Build jobs can be cached and are not a count of independently rerun tests. The
external score remains **832 checks across 13 families**.

`Leant2.synthesize : Query → MetaM Outcome` accepts frozen, closed native
expressions, honors an explicit provider inventory and axiom profile, and
restores caller Core/Meta state on all exits. Native rules and proof automation
remain available even with no ordinary providers. Returned terms and evidence
are checked again in the original environment; the API publishes no aliases,
declarations, compiler adapters, or executable definitions. See the
[library API guide](../../library-api.md).

Positive evidence must match the original target/contract pair or their exact
simultaneous zero-universe substitution, with independent program and contract
replay for that pair. The actual `programType` is preserved. Specialization may
affect only the contract, so matching the program type does not alone establish
the original query. Invalid input returns `preflightError`; invalid accepted
output raises an exception after restoration. A bounded miss is not logical
impossibility.

Upfront contract refutations now retain a portable certificate checked under
the requested profile after speculative state is restored. Its `program` is
the refutation and `programType` the negative statement; its separate `proof`
field is the trivial True witness. Newly created theorem bodies can be expanded
before replay, but fresh axioms, definitions, and opaque values cannot silently
escape. A strict-profile rejection falls through to ordinary search; the
bounded proof portfolio is not complete. The original negative statement is
preserved for dependency auditing even when reduction could erase an axiom.

The native [API tests](../../../tests/Leant2Tests/LibraryAPI.lean),
[contract-refutation tests](../../../tests/Leant2Tests/ContractRefutation.lean),
and [local-proof tests](../../../tests/Leant2Tests/LocalProof.lean) cover original
and specialized query replay, contract-only specialization, evidence/profile
checks, theorem extraction, and cancellation after speculative mutation. Their
build coverage is distinct from the external score and from the fresh frontend
processes counted below. Budgets remain cooperative, and external IO/work is not
undone by state restoration.
