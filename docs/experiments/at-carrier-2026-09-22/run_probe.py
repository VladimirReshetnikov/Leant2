#!/usr/bin/env python3
"""Run frozen at controls and both budgets; never build or rewrite sources.
...
def original_provenance():
    # Reconstruct the generated input bytes from the current authoritative
    # Python specification. This imports no Lean and starts no subprocess.
    source = importlib.util.spec_from_file_location("at_probe_generator", HERE / "prepare.py")
    generator = importlib.util.module_from_spec(source)
    source.loader.exec_module(generator)
    for filename, expected_bytes in generator.build_artifacts().items():
        if (HERE / filename).read_bytes() != expected_bytes:
            raise ValueError(f"generated artifact differs from exact source specification: {filename}")
    manifest = json.loads((HERE / "provenance.json").read_text(encoding="utf-8"))
    required = {
        "source_case_id": "church_case_033", "operation": "at", "source_line": 270,
        "observation_count": 168, "profile": "strictConstructive", "explicit_query_providers": ["BehaviorPartialNumeric.intCase"],
        "carrier": "Int → A", "open_holes_in_source_order": ["step", "init"],
    ...
    if len(observations) != 168 or len({row["id"] for row in observations}) != 168:
        raise ValueError("the original 168 distinct observations are required")
...
def classify(raw, controls: list[str] | None = None, budget: int | None = None):
    ...
    begin = f"case=church_case_033 operation=at observations=168 providers=1 profile=strictConstructive budgetMs={budget}"
    ...
    if outcome == "verified candidates=1":
        ...
        if one(terminal) != "exact_type_and_all_168_observations_replayed":
            problems.append("missing exact original-query acceptance marker")
        if one("PROBE_CANDIDATE") != "0 original_type_replayed=true original_observations_replayed=168 strict_axioms=0":
...
def main():
    ...
    parser.add_argument("--native-slot-released", action="store_true", required=True,
                        help="operator confirms the serialized native slot is available")
    ...
    declared = (ROOT / "lean-toolchain").read_text(encoding="utf-8").strip()
    if lean.parent.parent.name != declared.replace("/", "--").replace(":", "---"):
        parser.error("explicit Lean executable must belong to the declared pinned toolchain")
    ...
    controls = manifest["control_declarations"]
    if len(controls) != 11 or len(set(controls)) != 11:
        parser.error("expected the original eight witness/wrong and three primitive control audits")
    ...
    receipt = {
        ...
        "source_case_id": "church_case_033", "observation_count": 168, "budgets_ms": list(BUDGETS),
        "original_church_sha256_normalized": normalized, "original_spec_sha256": sha(spec),
        "native_slot_handoff_confirmed": True,
