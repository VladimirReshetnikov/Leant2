...
\begin{itemize}
\item \textbf{State coupling.} The cost of \code{saveState}/\code{restore} per attempt, heartbeat accounting across branches, and memory of retained frontiers are unmeasured. Mitigation: cursor frames, the two memory profiles, profiling at G1 before the scheduler is tuned\prov{\Ref{5},\Ref{9}}.
