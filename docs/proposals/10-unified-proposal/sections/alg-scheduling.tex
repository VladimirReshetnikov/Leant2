...
StepResult :=
  ...
  | checkedPrune (cert : Checked)                  -- exact completion set is empty
...
The cost vector of Section~\ref{sec:ranking} is used in three places: as the scalarized queue priority $g(j)$ of a task, as the ordering of accepted results under a profile, and as the basis of the Pareto frontier shown to the user\prov{\Pn{2},\Pn{3},\Pn{6},\Pn{7},\Pn{8}}.
\begin{lstlisting}
...
frontier.insert(t):
  if exists u in frontier with checkedEquivalent(u, t): merge evidence; return
  if exists u in frontier with dominates(u, t): return
  frontier := [u | u in frontier, not dominates(t, u)] ++ [t]
  if |frontier| > cap: drop the element with the worst scalar(., profile) among
                       those that are observationally indistinct from a kept one
\end{lstlisting}
