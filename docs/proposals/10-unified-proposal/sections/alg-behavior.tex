...
\end{lstlisting}
These constructors are schematic: each residual carries its exact native type, scope and dependency-closed environment. A hole occurrence includes its telescope substitution, including type, instance, universe, and index dependencies; identical hole identifiers with different captured substitutions are not interchangeable. Typedness alone does not establish evaluation correctness or safe pruning. Hard refutation additionally needs completion/substitution correspondence or conservative abstract semantics, preserving relevant dependencies even if reduction erases unused arguments. The maintained proposal's R3 specifies these obligations. Fuel is explicit; \code{Unknown} is a first-class outcome and is never read as \code{false}\prov{\Pn{2},\Pn{3},\Pn{6}}. \Pn{2}'s residual forms (known constructors, closures, suspended applications, conditionals, named holes with observed arguments) are adopted verbatim; \Pn{6} adds the path assumptions to the hole record.
