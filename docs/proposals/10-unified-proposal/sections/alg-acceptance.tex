...
Forge's \code{runWorker} saves the full tactic state, runs one worker under \code{tryCatchRuntimeEx} so that heartbeat and deadline exceptions are caught like any other failure, and on apparent success audits the proof term before keeping the state. Leant2 adds three things: a \emph{mode}, a \emph{permitted assignment set}, and a \emph{budget} argument; in joint mode the assignments the worker made to permitted holes are captured and returned as a patch, and the state is restored either way so that the engine applies the patch through its own transaction\prov{\Pn{2},\Pn{7}}.
\begin{lstlisting}
runWorker (goal : MVarId) (worker : Worker) (mode : Joint | Frozen)
          (permitted : Set MVarId) (policy : AxiomPolicy) (budget : Budget)
    : TacticM (Except String (Expr x AssignmentPatch)) := do
  ...
  let outcome <- tryCatchRuntimeEx (withBudget budget do  -- limits are failures too
      ...
      if pf.hasExprMVar  then throw "REJECTED: proof still contains metavariables"
    ...
    (fun ex => pure (Except.error (<- ex.toMessageData.toString)))
  saved.restore (restoreInfo := true)   -- ALWAYS: assignments, lctx, goals, messages
  return outcome
\end{lstlisting}
...
The frozen mode closes the candidate before the goal is formed: every program hole that is still open is replaced by a rigid local parameter and the obligation becomes universally quantified over those parameters, so that ``there should be no computational holes'' is enforced structurally rather than by convention\prov{\Pn{2},\Pn{6}}.
\begin{lstlisting}
freezeGoal (Phi : Expr) (t : Expr) : MetaM (MVarId x Array FVarId) := do
  let holes := (collectMVars t).filter isProgramHole
  withRigidParams holes fun params => do           -- one fvar per open hole
    let t'  := t.replaceMVars holes params
    let g   <- mkFreshExprMVar (Phi.betaRev #[t'])  -- Phi applied to the frozen term
    pure (g.mvarId!, params)
\end{lstlisting}
...
onAssign (m : MVarId) := for o in waitingOn m do
  if (instantiateMVars o.goal.type).hasProgramHole then o.readiness := Deferred
  else o.readiness := Ready; requeue o          -- arithmetic/index goals wake now

schedulerTurn := if growthSinceLastProof >= G then
  pick the Ready obligation with least (tier, cost); run adapter at its tier
  match reply with
...
This is a ranking policy, not a pruning theorem: branches whose debt is expensive stay available in the fair cost layer, and a production heuristic that abandons them must say so in the incompleteness ledger\prov{\Pn{1}}.
