...
\]
where $g$ is charged cost, $\hat h$ an estimate of remaining construction, $\hat p$ an estimate of proof debt, and the last two terms penalize syntactic near-duplicates and reward obligations that mention rigid heads and concrete observations. This is a heuristic order, explicitly not an admissible A$^*$ heuristic\prov{\Pn{1},\Pn{5}}. Counting open goals is not a lower bound either: two goals sharing an index can both close after one assignment. An admissible estimate needs a proof against the same final objective, accounting for correlated obligations; zero is the conservative baseline. The maintained proposal's R1 develops stronger restricted bounds.
...
Three independent controls apply: logical work budgets (rule applications, unification steps, normalization steps, proof tasks, solver calls, candidate evaluations), wall-clock deadlines, and process-level limits (memory, output size, external process lifetime). Counters are monotone and charged even after rollback (Decision~\ref{dec:ledger}). Per-rule limits yield suspended branches; raising a proof budget never regenerates earlier terms. A deterministic mode fixes the environment, grammar, logical budgets, proposal inputs, ordering, policy epochs, and logical admission schedule; replay uses a completed logical checkpoint rather than promising the same result at every wall-clock deadline. A performance mode may return the first result from parallel workers and records which lane produced it. Sorting surviving results fixes their presentation order, not the set lost to first-result cancellation\prov{\Pn{3},\Pn{4},\Pn{9}}.
