...
abstractCompose(goalType, inventory, k):
  A := abstraction with nodes = (rigid head, bounded argument pattern), edges = providers
  loop up to k times:
    path := shortestPath(A, abs(availableTypes), abs(goalType))
    if none: return noProposal            -- advisory only, never negative evidence
    t := elaborate path natively in the branch (spines of alg:search, transports of alg:transport)
    if t typechecks: return proposal t
    d := explain mismatch (index Vec A 0 vs Vec A (n+1), universe, dictionary identity)
    A := refine A by the distinction d       -- split one node; never ban the provider
  return noProposal
\end{lstlisting}

The abstraction keeps nominal heads, index dependencies, universe constraints, and dictionary identities as labels so that a failed path yields a concrete refinement; the exact branch stays authoritative and no completeness theorem is claimed for the abstract graph\prov{\Pn{4},\Pn{9}}.
