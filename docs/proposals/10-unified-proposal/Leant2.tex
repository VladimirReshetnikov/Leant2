...
Part~I fixes the architecture. Part~II gives the algorithms: the construction search and spine expansion, dependent elimination and transport, retrieval and carrier invention, recursion-schema and motive synthesis, contract propagation and certified counterexample-guided refinement, scheduling and memoization, and the proof-service and acceptance procedures, each as pseudocode merged from the best-specified proposals, with the worked traces that recur across them. Two requirements come from the project owner rather than from the proposals. First, Leant2 has a single adaptive engine: no engine switches and, for now, no user-facing settings. Second, the minimal acceptance baseline is the existing Leant \code{:synth} golden corpus, thirty transcripts with 278 queries, read semantically: Leant2 must match or exceed every recorded outcome, but may return better candidates in a different order. Nothing here adds new experimental evidence; the evidence sections cite the nine packages and the sibling project Forge, whose kernel-checked certificate checkers and orchestration frontend Leant2 should reuse.
\end{abstract}
...
\paragraph{Maintained continuation.} This document preserves the architecture and historical evidence of proposals P1--P9. The \href{../11-further-improvements/Leant2-next.pdf}{maintained further-improvements proposal} integrates the later N1--N9 expert answers. Its R1 and R3--R9 refine the obligations for search bounds, typed residuals, recursion realization, proof services, retrieval, ranking, and proposal replay. These consistency corrections add no implementation or performance evidence; what the original prototypes demonstrated remains revision-bound.
...
\input{sections/01-scope-baseline}
\input{sections/02-decisions-problem}
\input{sections/03-state-search}
\input{sections/04-construction}
\input{sections/05-recursion}
\input{sections/06-behavior}
\input{sections/07-acceptance-deployment}

\part{Algorithms}\label{part:algorithms}
\input{sections/alg-search}
\input{sections/alg-elim-retrieval}
\input{sections/alg-recursion}
\input{sections/alg-behavior}
\input{sections/alg-scheduling}
\input{sections/alg-acceptance}

\part{Evidence, evaluation, and roadmap}\label{part:evidence}
\input{sections/08-evidence-evaluation-roadmap}
