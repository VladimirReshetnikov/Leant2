...
def BExpr.eval : BExpr → Bool → Bool → Bool
  | .x, leftValue, _ => leftValue
  | .y, _, rightValue => rightValue
  | .neg a, leftValue, rightValue => !(a.eval leftValue rightValue)
  | .conj a b, leftValue, rightValue =>
    a.eval leftValue rightValue && b.eval leftValue rightValue
  | .disj a b, leftValue, rightValue =>
    a.eval leftValue rightValue || b.eval leftValue rightValue
